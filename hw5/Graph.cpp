...
void Graph::findNextEdge(vector<Edge> &ans) {
    ...
    for (int i = 0; i < static_cast<int>(myList[temp_min.FirstNode].size());
         ++i) {
        if (myList[temp_min.FirstNode][i].index == temp_min.SecondNode) {
            myList[temp_min.FirstNode].erase(
                myList[temp_min.FirstNode].begin() + i);
            break;
        }
    }

    for (int i = 0; i < static_cast<int>(myList[temp_min.SecondNode].size());
         ++i) {
        if (myList[temp_min.SecondNode][i].index == temp_min.FirstNode) {
            myList[temp_min.SecondNode].erase(
                myList[temp_min.SecondNode].begin() + i);
            break;
        }
    }
