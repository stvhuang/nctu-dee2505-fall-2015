...
void solve::findCombinations( vector <int>& candidates, int startPos, int target, vector<int>& oneAns, vector<vector <int> >& solutions, int useOfNeg) {
   if ( target == 0 ) {
   ...
   for ( int i(startPos); i < numOfCandidates; ++i ) {
       ...
       if( candidates[i] <= target && useOfNeg <= limOfNeg ) {
           if ( candidates[i] >= 0 ) {
               ...
               findCombinations( candidates, i, target - candidates[i], oneAns, solutions, useOfNeg );
               oneAns.pop_back();
