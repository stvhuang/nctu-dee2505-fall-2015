...
struct Edge {
    Edge( int F = 0, int S = 0, int W = 0) : FirstNode(F), SecondNode(S), weight(W) {};
    ...
    bool operator==( const Edge arg ) {
        if ( ( this->FirstNode == arg.FirstNode ) && ( this->SecondNode == arg.SecondNode ) && ( this->weight == arg.weight ) ) return true;
        return false;
    }
